...
class MainActivity : FragmentActivity() {
    ...
    private var languageCode by mutableStateOf("ar")
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val appearance = getSharedPreferences("yonte_appearance", MODE_PRIVATE)
        darkTheme = appearance.getBoolean("dark_theme", true)
        languageCode = appearance.getString("language", "ar").takeIf { it == "en" } ?: "ar"
        ...
        setContent {
            ...
            CompositionLocalProvider(LocalLayoutDirection provides if (languageCode == "ar") LayoutDirection.Rtl else LayoutDirection.Ltr) {
    ...
    private fun NotesOrSettings() {
        ...
        ModalNavigationDrawer(
            ...
            drawerContent = {
                ModalDrawerSheet(Modifier.widthIn(max = 420.dp).fillMaxWidth(0.92f)) {
                    SettingsRoute(
                        ...
                        onThemeChanged = {
                            darkTheme = it
                            getSharedPreferences("yonte_appearance", MODE_PRIVATE).edit().putBoolean("dark_theme", it).apply()
                        },
                        languageCode = languageCode,
                        onLanguageChanged = {
                            if (it == "ar" || it == "en") {
                                languageCode = it
                                getSharedPreferences("yonte_appearance", MODE_PRIVATE).edit().putString("language", it).apply()
                            }
                        },
            ...
            NotesRoute(
                ...
                onSharedTextConsumed = {
                    // The ViewModel has captured the draft. Do not replay the same
                    // ACTION_SEND payload when this Activity is recreated.
