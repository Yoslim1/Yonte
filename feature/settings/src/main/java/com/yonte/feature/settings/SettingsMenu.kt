    ...
    Column(Modifier.fillMaxWidth().verticalScroll(rememberScrollState()).padding(20.dp), verticalArrangement = Arrangement.spacedBy(16.dp)) {
        Surface(shape = MaterialTheme.shapes.large, color = MaterialTheme.colorScheme.primaryContainer.copy(alpha = 0.58f)) {
            Column(Modifier.fillMaxWidth().padding(20.dp)) {
                ...
                Text(if (isArabic) "مساحة هادئة لأفكارك" else "A calm space for your thoughts", color = MaterialTheme.colorScheme.onSurfaceVariant)
            }
        }
        ...
        SettingsAppearance(darkTheme, onThemeChanged, isArabic, languageCode, onLanguageChanged)
        SettingsMenuRow(if (isArabic) "النسخ الاحتياطي" else "Backup", if (isArabic) "تصدير واستيراد نسخة محلية" else "Export and import a local backup", Icons.Outlined.FolderZip, onData)
